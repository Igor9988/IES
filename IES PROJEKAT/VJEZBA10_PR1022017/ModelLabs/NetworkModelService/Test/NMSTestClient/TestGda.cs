    ...
    public class TestGda : IDisposable
    {
        ...
        public List<long> GetExtentValues(ModelCode modelCode)
        {
            ...
            try
            {
                ...
                if (modelCode.Equals(ModelCode.POWERTR))
                {
                    xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetPTs.xml", Encoding.Unicode);
                }
                else
                {
        ...
        // zad 1
        public IEnumerable<long> GetPTValues()
		{
            return this.GetExtentValues(ModelCode.POWERTR);
		}

        // zad2 2
        public IEnumerable<long> GetTWValues()
		{
            return this.GetRelatedValues(this.GetPTValues().Min(), new Association() { PropertyId = ModelCode.POWERTR_WINDINGS, Type = ModelCode.POWERTRWINDING });
		}
        ...
        public List<long> GetRelatedValues(long sourceGlobalId, Association association)
        {
            ...
            try
            {
                ...
                if (association.Type.Equals(ModelCode.POWERTRWINDING))
                {
                    xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetWindings.xml", Encoding.Unicode);
                }
                else
                {
                    xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetRelatedValues_Results.xml", Encoding.Unicode);
